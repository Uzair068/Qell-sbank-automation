...
Call log:
  - waiting for locator('input[name="uid"]') to be visible
...
```ts
  ...
  7  |   await page.goto('/');
  8  |   
  9  |   // Wait for login inputs to be visible
> 10 |   await page.locator('input[name="uid"]').waitFor({ state: 'visible', timeout: 10000 });
     |                                           ^ TimeoutError: locator.waitFor: Timeout 10000ms exceeded.
  11 | 
  12 |   // Login
  13 |   await page.locator('input[name="uid"]').fill(process.env.MANAGER_ID!);
  14 |   await page.locator('input[name="password"]').fill(process.env.MANAGER_PASSWORD!);
  15 |   await page.locator('input[name="btnLogin"]').click();
  16 | 
  17 |   // Confirm logged in
  18 |   await expect(page.locator('td[class="heading3"]'))
  19 |     .toContainText('Manger Id : mngr660320');
  20 | 
  21 |   // Save session
