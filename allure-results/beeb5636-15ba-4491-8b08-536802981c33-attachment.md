...
```
TypeError: page.getByName is not a function
```
...
```ts
  ...
  8  | 
  9  |   // Login
> 10 |   await page.getByName('uid').fill(process.env.MANAGER_ID!);
     |              ^ TypeError: page.getByName is not a function
  11 |   await page.getByName('password').fill(process.env.MANAGER_PASSWORD!);
  12 |   await page.getByName('btnLogin').click();
  13 | 
